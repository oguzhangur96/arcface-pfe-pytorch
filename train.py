...
from models.mls_loss import MLSLoss
from data.verify import VerifyFace
from data.casia_pfe import CASIAWebFacePFE
...
class MetricFace(VerifyFace):

    def __init__(self, args):

        VerifyFace.__init__(self, args)
        self.args = args
        self.model = dict()
        self.data = dict()
    ...
    def _model_loader(self):
        ...
        self.model['backbone'] = MobileFaceNet(512)
        save_path = '/home/oguz/workspace/projects/arcface-pfe-torch/pretrained/model_mobilefacenet.pth'
        self.model['backbone'].load_state_dict(torch.load(save_path))
        self.model['uncertain'] = UncertaintyHead(512)
        self.model['criterion'] = MLSLoss(mean=False)
    ...
    def _data_loader(self):
        self.data['train_loader'] = DataLoader(
            CASIAWebFacePFE(self.args, mode='train'),
